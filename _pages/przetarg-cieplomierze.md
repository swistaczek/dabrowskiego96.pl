---
permalink: /przetarg-cieplomierze/
title: "Przetarg - Montaż 44 Ciepłomierzy | Oszczędność 15-25% | Dąbrowskiego 96"
description: "Wymiana 123 podzielników na 44 ciepłomierze ultradźwiękowe. Precyzyjne rozliczanie, oszczędność 15-25% na ogrzewaniu. Zapytanie ofertowe dla wykonawców."
keywords: "ciepłomierze Poznań, wymiana podzielników, ciepłomierze ultradźwiękowe, montaż ciepłomierzy, przetarg ogrzewanie"
image: /assets/images/cieplomierze-montaz.jpg
toc: true
---

![Montaż ciepłomierzy ultradźwiękowych](/assets/images/cieplomierze-montaz.jpg)

**Wspólnota Mieszkaniowa Dąbrowskiego 96** ogłasza zapytanie ofertowe na dostawę i montaż ciepłomierzy ultradźwiękowych w budynku przy ul. Dąbrowskiego 96 w Poznaniu.

## 1. Przedmiot zamówienia

### Obecny stan instalacji

| Parametr | Wartość |
|----------|--------|
| Lokale z ciepłomierzami | 5 |
| Lokale z podzielnikami | 39 |
| Łączna liczba podzielników | 123 |
| Planowana liczba ciepłomierzy | 44 |

### Stan istniejący szachtów instalacyjnych

Poniższe zdjęcia przedstawiają fragmenty szachtów instalacyjnych w kamienicy, w których będą montowane ciepłomierze:

<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0;">

<div>
<img src="/assets/images/szacht-instalacyjny-1.jpg" alt="Szacht instalacyjny z pionowymi rurami i zaworami odcinającymi" style="width: 100%; border-radius: 8px;">
<p style="font-size: 14px; color: #666; margin-top: 10px;"><strong>Szacht główny:</strong> Pionowe prowadzenie rur wodnych z zaworami odcinającymi (czerwone uchwyty). Widoczne numeracje lokali i izolacja termiczna.</p>
</div>

<div>
<img src="/assets/images/szacht-instalacyjny-2.jpg" alt="Szacht z metalowymi drzwiczkami" style="width: 100%; border-radius: 8px;">
<p style="font-size: 14px; color: #666; margin-top: 10px;"><strong>Szacht z drzwiczkami:</strong> Metalowe drzwiczki umożliwiające dostęp do instalacji. Widoczne czerwone uchwyty zaworów.</p>
</div>

<div>
<img src="/assets/images/szacht-instalacyjny-3.jpg" alt="Szacht z trzema poziomami instalacji" style="width: 100%; border-radius: 8px;">
<p style="font-size: 14px; color: #666; margin-top: 10px;"><strong>Układ trójpoziomowy:</strong> Szacht z otwartymi drzwiczkami, pokazujący trzy poziomy instalacji z czerwonymi zaworami kulowymi i numeracją.</p>
</div>

<div>
<img src="/assets/images/szacht-instalacyjny-4.jpg" alt="Kolejny szacht z układem trójpoziomowym" style="width: 100%; border-radius: 8px;">
<p style="font-size: 14px; color: #666; margin-top: 10px;"><strong>Podobny układ:</strong> Trzy poziomy instalacji z białymi rurami PCV, zaworami odcinającymi i czytelną numeracją lokali.</p>
</div>

</div>

**Uwagi dotyczące montażu:**
- Szachty są łatwo dostępne i posiadają wystarczającą przestrzeń roboczą
- Istniejące zawory odcinające pozwalają na izolację podczas montażu
- Czytelna numeracja ułatwia identyfikację przyłączy do poszczególnych lokali
- Stan techniczny instalacji jest dobry i nie wymaga dodatkowych prac przygotowawczych

### Zakres prac do wykonania

1. **Demontaż** 123 podzielników ciepła
2. **Dostawa** 44 ciepłomierzy ultradźwiękowych
3. **Montaż** i podłączenie liczników
4. **Legalizacja** wszystkich urządzeń
5. **Integracja** z istniejącymi instalacjami grzewczymi
6. **Konfiguracja** systemu odczytu
7. **Uruchomienie** i testy systemu
8. **Szkolenie** zarządcy z obsługi

## 2. Dlaczego wymieniamy podzielniki?

### Porównanie systemów

| Cecha | Podzielniki ciepła | Ciepłomierze |
|-------|-------------------|--------------|
| **Dokładność pomiaru** | Szacunkowa (jednostki) | Rzeczywista (kWh) |
| **Błąd pomiaru** | Do 30% | Do 2% |
| **Wpływ lokalizacji** | Bardzo duży | Minimalny |
| **Koszty rozliczeń** | Wyższe | Niższe o 15-25% |
| **Zgodność z prawem UE** | Wycofywane | Rekomendowane |

### Korzyści dla mieszkańców

✅ **Sprawiedliwe rozliczenia** - płacisz tylko za rzeczywiste zużycie  
✅ **Niższe rachunki** - średnio 15-25% oszczędności  
✅ **Przejrzystość** - odczyty w kWh, łatwe do weryfikacji  
✅ **Kontrola zużycia** - możliwość bieżącego monitorowania  

## 3. Wymagania techniczne

### Parametry ciepłomierzy

- Typ: **ultradźwiękowe** (bezruchome, długa żywotność)
- Klasa dokładności: minimum **2** wg PN-EN 1434
- Przepływ nominalny: dostosowany do instalacji (qp 0,6-2,5 m³/h)
- Zasilanie: bateria o żywotności min. **10 lat**
- Komunikacja: **M-Bus** lub radiowa
- Certyfikaty: MID, deklaracja zgodności CE

### Wymagania instalacyjne

- Montaż zgodny z DTR producenta
- Zachowanie prostych odcinków przed/za licznikiem
- Plombowanie liczników i przyłączy
- Dokumentacja fotograficzna każdego punktu pomiarowego

## 4. Wymagania dla wykonawcy

### Doświadczenie
- Minimum **3 lata** w montażu ciepłomierzy
- Referencje z minimum **2 wspólnot mieszkaniowych** (powyżej 40 lokali)
- Certyfikat autoryzowanego instalatora od producenta urządzeń

### Dokumenty
- Aktualna polisa OC (min. 500 000 PLN)
- Wpis do rejestru działalności gospodarczej
- Uprawnienia instalacyjne
- Certyfikaty jakości oferowanych urządzeń

### Referencje po wykonaniu prac
✅ **Wspólnota wystawia referencje** dla wykonawców, którzy rzetelnie i terminowo zrealizują zamówienie. Referencje mogą być wykorzystane w przyszłych przetargach i postępowaniach ofertowych.

## 5. Harmonogram realizacji

| Etap | Czas realizacji |
|------|----------------|
| Przygotowanie dokumentacji | 7 dni |
| Dostawa urządzeń | 14 dni |
| Montaż i uruchomienie | 7-10 dni |
| Testy i odbiór | 3 dni |
| **RAZEM** | **ok. 30 dni** |

## 6. Kryteria oceny ofert

| Kryterium | Waga |
|-----------|------|
| Cena całkowita | 40% |
| Jakość urządzeń (klasa, funkcje) | 25% |
| Doświadczenie wykonawcy | 20% |
| Warunki gwarancji i serwisu | 10% |
| Termin realizacji | 5% |

## 7. Zawartość oferty

Oferta powinna zawierać:

1. **Formularz ofertowy** z ceną netto i brutto
2. **Specyfikację techniczną** oferowanych ciepłomierzy
3. **Kosztorys szczegółowy**:
   - Cena jednostkowa ciepłomierza
   - Koszt montażu na punkt
   - Koszt demontażu podzielników
   - Koszt uruchomienia systemu
4. **Harmonogram** prac
5. **Referencje** (minimum 2)
6. **Dokumenty** wymienione w pkt. 4
7. **Warunki gwarancji** i serwisu pogwarancyjnego

## 8. Termin i sposób składania ofert

📅 **Termin:** do uzgodnienia indywidualnie  
📧 **E-mail:** biuro@nieruchomoscipk.pl  
📋 **Temat:** "Oferta - ciepłomierze Dąbrowskiego 96"

## 9. Wizja lokalna

Zalecamy przeprowadzenie wizji lokalnej w celu:
- Oceny stanu instalacji i szachtów instalacyjnych
- Weryfikacji ilości i lokalizacji punktów pomiarowych
- Określenia optymalnych miejsc montażu ciepłomierzy
- **Oględzin szachtów instalacyjnych** - dostęp do wszystkich szachtów przedstawionych na zdjęciach powyżej
- Sprawdzenia przestrzeni roboczej i warunków montażu
- Weryfikacji kompatybilności z istniejącymi zaworami odcinającymi

**Dodatkowe informacje:**
- Szachty instalacyjne są łatwo dostępne przez cały dzień roboczy
- Możliwość dokładnych pomiarów i fotografowania dla potrzeb oferty
- Koordynator zapewni dostęp do wszystkich pomieszczeń i szachtów

Termin wizji do uzgodnienia z koordynatorem.

## 10. Kontakt

### Koordynator projektu
**Patrycja Kucharska**  
Tel: +48 507 369 487  
E-mail: biuro@nieruchomoscipk.pl

### Zarząd Wspólnoty
E-mail: zarzad@dabrowskiego96.pl

## 11. FAQ - Najczęściej zadawane pytania

**P: Czy montaż wymaga wyłączenia ogrzewania?**  
O: Tak, na krótki czas (ok. 30-60 minut na lokal). Prace planowane są poza sezonem grzewczym.

**P: Czy będę mógł sam odczytywać wskazania?**  
O: Tak, każdy ciepłomierz ma czytelny wyświetlacz LCD.

**P: Jak długo trwa legalizacja?**  
O: Pierwotna legalizacja to 5 lat, można ją przedłużyć o kolejne 5 lat.

**P: Czy cena obejmuje wszystkie koszty?**  
O: Oferta powinna zawierać kompletną cenę "pod klucz" wraz z wszystkimi materiałami.

---

*Wspólnota zastrzega sobie prawo do wyboru najkorzystniejszej oferty, negocjacji warunków oraz unieważnienia postępowania bez podania przyczyny.*